...
public class PositionDAO {
    ...
    public List<Position> getAllPositions() {
        ...
        String sql = "SELECT p.position_id, p.position_name, p.department_id, p.description, p.status, p.is_deleted, p.created_at, "
                   + "d.department_name, "
    ...
    public boolean addPosition(Position pos) {
        String sql = "INSERT INTO Positions (position_name, department_id, description, status, is_deleted) VALUES (?, ?, ?, ?, 0)";
        try (Connection conn = DBContext.getInstance().getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public boolean updatePosition(Position pos) {
        String sql = "UPDATE Positions SET position_name = ?, department_id = ?, description = ?, status = ? WHERE position_id = ? AND is_deleted = 0";
        try (Connection conn = DBContext.getInstance().getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, pos.getPositionName());
            if (pos.getDepartmentId() != null && pos.getDepartmentId() > 0) {
    ...
    public boolean deletePosition(int id) {
        String sql = "UPDATE Positions SET is_deleted = 1 WHERE position_id = ?";
        try (Connection conn = DBContext.getInstance().getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, id);
            return ps.executeUpdate() > 0;
    ...
    public List<JobLevel> getAllJobLevels() {
        ...
        String sql = "SELECT level_id, level_name, rank_order, description FROM Job_Levels ORDER BY rank_order ASC";
        try (Connection conn = DBContext.getInstance().getConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
    ...
    public int countPositions() {
        String sql = "SELECT COUNT(*) FROM Positions WHERE is_deleted = 0 AND status = 1";
        try (Connection conn = DBContext.getInstance().getConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
